/**
 * @file VideoSettings.tsx
 * @description 视频生成提供商设置面板（移动端）。
 *
 * 允许配置 API Key、Base URL、启用状态、模型列表 CRUD（新增 / 编辑 / 删除），
 * 并支持调用后端 `/api/generate/video` 接口进行连通性测试。
 *
 * 设计说明：
 * - 与 ImageSettings 结构一致，但调用视频生成测试接口。
 * - 由于 `settingsStore` 仅暴露 `setVideoProviderConfig`，无法直接修改全局
 *   `videoModelId`，因此当前选中的模型会被持久化到该提供商的
...
/** 视频生成测试 API 路径（相对于 API origin）。 */
const VIDEO_TEST_PATH = '/api/generate/video';
...
export function VideoSettings({ selectedProviderId }: VideoSettingsProps) {
  ...
  const providerConfig = videoProvidersConfig[selectedProviderId];
  const providerMeta = VIDEO_PROVIDERS[selectedProviderId];
  ...
  const handleSaveModel = (): void => {
    ...
    setVideoProviderConfig(selectedProviderId, { customModels: nextModels });
    setSheetOpen(false);
  };
  ...
  const handleTest = async (): Promise<void> => {
    ...
    setTestMessage('');
    try {
      const response = await fetch(resolveVideoTestUrl(), {
        method: 'POST',
      ...
      <Card style={styles.section}>
        <View style={styles.rowBetween}>
          <View style={styles.flex1}>
            <Label>启用此提供商</Label>
            <Text style={styles.hint}>启用后可在视频生成功能中使用</Text>
          </View>
          <Switch
            value={providerConfig.enabled}
            onValueChange={(checked) =>
              setVideoProviderConfig(selectedProviderId, { enabled: checked })
            }
          />
        </View>
      </Card>
            ...
            <View style={styles.row}>
              <View style={styles.flex1}>
                <Input
                  ...
                  onChangeText={(text) =>
                    setVideoProviderConfig(selectedProviderId, { apiKey: text })
                  }
