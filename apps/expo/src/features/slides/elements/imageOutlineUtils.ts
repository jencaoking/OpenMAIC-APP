...
export function getElementOutline(outline?: {
  ...
  if (outlineStyle === 'dashed') {
    strokeDasharray = outlineWidth <= 6
...
export const CLIP_PATHS: Record<string, (w: number, h: number) => string> = {
  rect2: (w, h) =>
    `M 0 0 L ${w * 0.8} 0 L ${w} ${h * 0.2} L ${w} ${h} L 0 ${h} Z`,
  rect3: (w, h) =>
  ...
  rhombus: (w, h) =>
    `M ${w * 0.5} 0 L ${w} ${h * 0.5} L ${w * 0.5} ${h} L 0 ${h * 0.5} Z`,
  pentagon: (w, h) =>
  ...
  point: (w, h) =>
    `M 0 0 L ${w * 0.75} 0 L ${w} ${h * 0.5} L ${w * 0.75} ${h} L 0 ${h} Z`,
  arrow: (w, h) =>
    `M 0 ${h * 0.2} L ${w * 0.6} ${h * 0.2} L ${w * 0.6} 0 L ${w} ${h * 0.5} L ${w * 0.6} ${h} L ${w * 0.6} ${h * 0.8} L 0 ${h * 0.8} Z`,
  parallelogram: (w, h) =>
    `M ${w * 0.3} 0 L ${w} 0 L ${w * 0.7} ${h} L 0 ${h} Z`,
  parallelogram2: (w, h) =>
    `M ${w * 0.3} ${h} L ${w} ${h} L ${w * 0.7} 0 L 0 0 Z`,
  trapezoid: (w, h) =>
    `M ${w * 0.25} 0 L ${w * 0.75} 0 L ${w} ${h} L 0 ${h} Z`,
  trapezoid2: (w, h) =>
    `M 0 0 L ${w} 0 L ${w * 0.75} ${h} L ${w * 0.25} ${h} Z`,
};
